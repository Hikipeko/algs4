...
public class Selection_ {
    ...
    private static int exchTimes;

    public static int exchTimes() {
        return exchTimes;
    }
    ...
    public static void draw(Comparable[] a, int min, int row, int ith) {
        StdDraw.setPenColor(StdDraw.BLACK);
        StdDraw.text(-2.5, row, ith + "");
        StdDraw.text(-1.25, row, min + "");
        for (int i = 0; i < a.length; i++) {
            if (i < ith) StdDraw.setPenColor(StdDraw.GRAY);
            else if (i == min) StdDraw.setPenColor(StdDraw.RED);
